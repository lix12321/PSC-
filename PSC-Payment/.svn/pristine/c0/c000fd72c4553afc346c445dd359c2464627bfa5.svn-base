...
import cn.wellcare.handler.transaction.payment.wechat.WechatNativeBasePayHandler;
 ...
 *
 */
@Service("wechatNativePaymentService")
public class WechatNativeService extends WechatPayment implements PaymentApi {
	protected Logger log = Logger.getLogger(this.getClass());
	@Resource
	private WechatNativeBasePayHandler wechatNativeBasePayHandler;

	@Override
	protected String getWechatOrderType() {
		return PaymentType.WECHAT_NATIVE.getPaymentCode();
	}

	@Override
	protected String getWechatPaymentName() {
		return PaymentType.WECHAT_NATIVE.getPaymentName();
	}
	...
	public RpcResult<PaymentResult> doPay(Map<String, Object> param) {
		...
		try {
			...
			// 2.支付
			sr.setData(wechatNativeBasePayHandler.doPay(param));
		} catch (Exception e) {
			sr.setSuccess(false);
