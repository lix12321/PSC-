...
public class PayLogController {
	...
	public @ResponseBody HttpJsonResult<List<PayLog>> list(HttpServletRequest request, ModelMap dataMap) {
		Map<String, Object> queryMap = WebUtil.handlerQueryMap(request);
		PagerInfo pager = WebUtil.handlerPagerInfo(request, dataMap);
		//获取登录人信息
		SystemUsers adminUser = SessionManager.getAdminUser(request);
		queryMap.put("orgId",adminUser.getId());
		RpcResult<List<PayLog>> serviceResult = payLogService.page(queryMap, pager);
		...
		HttpJsonResult<List<PayLog>> jsonResult = new HttpJsonResult<List<PayLog>>();
	...
	public String edit(HttpServletRequest request, ModelMap dataMap, Integer id) {
		if (id != null) {
			PayLog payLog = payLogService.getPayLogById(id).getData();
			dataMap.put("obj", payLog);
		}

		return "/admin/payLog/payLogedit";
	}
	...
	public @ResponseBody HttpJsonResult<Boolean> doAdd(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			if (payLog.getId() != null && 0 != payLog.getId()) {
				serviceResult = payLogService.updatePayLog(payLog);
			} else {
				serviceResult = payLogService.savePayLog(payLog);
			}
	...
	public @ResponseBody HttpJsonResult<Boolean> del(HttpServletRequest request, HttpServletResponse response,
		...
		HttpJsonResult<Boolean> jsonResult = new HttpJsonResult<Boolean>();
		try {
			RpcResult<Boolean> serviceResult = payLogService.del(id);
			if (!serviceResult.isSuccess()) {
				if (Constants.SERVICE_RESULT_CODE_SYSERROR.equals(serviceResult.getMsgCode())) {
					throw new RuntimeException(serviceResult.getMsgInfo());
				} else {
