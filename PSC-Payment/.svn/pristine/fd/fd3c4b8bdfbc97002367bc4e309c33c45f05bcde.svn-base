 ...
 * 支付设置-微信相关action
 *                       
 * @Filename: PaySettingWechat.java
 * @Version: 1.0
 * 
 */
@Controller
@RequestMapping(value = "paysettingwechat")
public class PaySettingWechatController  {
    @Resource
    private IPaySettingWechatService paySettingWechatService;
    ...
    public String index(HttpServletRequest request, ModelMap dataMap) throws Exception {
        ...
        return "pay/paysettingwechatlist";
    }
    ...
    public @ResponseBody HttpJsonResult<List<PaySettingWechat>> list(HttpServletRequest request,
                                                                     ModelMap dataMap) {
        Map<String, Object> queryMap = WebUtil.handlerQueryMap(request);
        ...
        RpcResult<List<PaySettingWechat>> serviceResult = paySettingWechatService.page(
            queryMap, pager);
        ...
        HttpJsonResult<List<PaySettingWechat>> jsonResult = new HttpJsonResult<List<PaySettingWechat>>();
    ...
    public String add(HttpServletRequest request, ModelMap dataMap, Integer id) {
        return "pay/paysettingwechatadd";
    }

    @RequestMapping(value = "edit", method = { RequestMethod.GET })
    public String edit(HttpServletRequest request, ModelMap dataMap, Integer id) {
        if (id != null) {
            PaySettingWechat paySettingWechat = paySettingWechatService.getPaySettingWechatById(id)
                .getData();
            dataMap.put("obj", paySettingWechat);
        }

        return "pay/paysettingwechatedit";
    }
    ...
    public @ResponseBody HttpJsonResult<Boolean> doAdd(HttpServletRequest request,
                                                       ...
                                                       PaySettingWechat paySettingWechat) {
        HttpJsonResult<Boolean> jsonResult = new HttpJsonResult<Boolean>();
        //获取登录人信息
		SystemUsers adminUser = SessionManager.getAdminUser(request);
        RpcResult<Integer> serviceResult = null;
        try {
            if (paySettingWechat.getId() != null && 0 != paySettingWechat.getId()) {
                serviceResult = paySettingWechatService.updatePaySettingWechat(paySettingWechat);
            } else {
                if (null != adminUser){
                    paySettingWechat.setOrgId(adminUser.getId());
                }
                serviceResult = paySettingWechatService.savePaySettingWechat(paySettingWechat);
            }
    ...
    public @ResponseBody HttpJsonResult<Boolean> del(HttpServletRequest request,
        ...
        HttpJsonResult<Boolean> jsonResult = new HttpJsonResult<Boolean>();
        try {
            RpcResult<Boolean> serviceResult = paySettingWechatService.del(id);
           if (!serviceResult.isSuccess()) {
                if (Constants.SERVICE_RESULT_CODE_SYSERROR.equals(serviceResult.getMsgCode())) {
					throw new RuntimeException(serviceResult.getMsgInfo());
				} else {
