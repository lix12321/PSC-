...
public class CommonCaptcha {
	...
	// 字母、数字组合
	public static final int TYPE_CHAR_NUM = 3;
	// 数字
	public static final int TYPE_NUM = 4;

	public void getCaptcha(HttpServletRequest request, HttpServletResponse response) throws IOException {
		getCaptcha(request, response, TYPE_CHAR_NUM);
	}

	public void getCaptcha(HttpServletRequest request, HttpServletResponse response, Integer type) throws IOException {
		// 在内存中创建一张图片
		...
		// 将随机数存在session中
		SessionManager.setVerifyNumber(request, random);
	 ...
	 */
	private String drawRandomNum(Graphics2D g, Integer type) {
		// 设置颜色
		g.setColor(new Color(53, 64, 77));
		// 设置字体
		g.setFont(new Font("Verdana", Font.ITALIC | Font.BOLD, 30));
		// 常用的中国汉字
		...
		if (type != null) {
			if (TYPE_CHINESE == type) {
				// 截取汉字
				return createRandomChar(g, baseChineseChar);
			} else if (TYPE_CHAR_NUM == type) {
				// 截取数字和字母的组合
				return createRandomChar(g, baseNumLetter);
			} else if (TYPE_NUM == type) {
				// 截取数字
