package cn.wellcare.rpc.filter;
...
public class PscDubboExceptionFilter implements Filter {
		...
		try {
			...
			if (result.hasException() && GenericService.class != invoker.getInterface()) {
				try {
					...
					// 项目异常，抛出
					if (className.startsWith("cn.wellcare.core.exception.")) {
						logger.debug("RPC业务异常：" + className);
						return result;
					}
